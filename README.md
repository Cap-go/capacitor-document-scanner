...
| **`maxNumDocuments`**     | <code>number</code>                                   | Maximum number of documents to scan. On iOS: VisionKit caps scans at 24 pages (system limit). On Android: customizable limit; defaults to 20 for performance (clamped 1-24). Set to 1 for single-scan mode where the scanner stops after one document.                                           | <code>20 on Android, 24 on iOS</code>   |
